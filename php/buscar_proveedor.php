<?php 
  ...
  if($post['search'] == 'true'){
    ...
    $search['like']=elements(array('empresa','contacto'),$_REQUEST);
    ...
    $search['where'] = elements(array('empresa','contacto'),$_REQUEST);
  ...
  if (!$se) {
    $sql = "select clave_proveedor,empresa,contacto,concat(calle,' ',numero,' ',colonia) as direccion,telefono,celular,status 
      from proveedores
      where  status = 'ACTIVO'";
  ...
  while($row = mysql_fetch_object($query)){
    $result[$i]['matricula'] = $row->clave_proveedor;
    $result[$i]['cell'] = array($row->clave_proveedor,$row->empresa,$row->contacto,
    $row->direccion,$row->telefono,$row->celular,$row->status);
    $i++;
  }     
