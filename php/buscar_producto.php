<?php 
  ...
  if($post['search'] == 'true'){
    ...
    $search['like'] = elements(array('nombre_producto','proveedor'),$_REQUEST);
    ...
    $search['where'] = elements(array('nombre_producto','proveedor'),$_REQUEST);
        //HACIENDO UN RECORRIDO SOBRE ELLOS CREAMOS LA CONSULTA.
  ...
  $count = mysql_result($query,0);
	...
	if (!$se) {
    	...
    	$sql = "select clave_producto,nombre_producto,empresa as proveedor,nombre_medida,
			cantidad_actual,precio,productos.status 
			from productos
			inner join proveedores p on p.clave_proveedor = productos.proveedor
			inner join unidad_medida um on  um.clave_medida = productos.unidad_medida";
		...
		$sql =" select clave_producto,nombre_producto,empresa as proveedor,nombre_medida,
			...
			from productos
			inner join proveedores p on p.clave_proveedor = productos.proveedor
			inner join unidad_medida um on  um.clave_medida = productos.unidad_medida".$se;
    	if( !empty($post['orden']) && !empty($post['orderby'])) {
	...
	while($row = mysql_fetch_object($query)){
		$result[$i]['clave'] = $row->clave_producto;
		$result[$i]['cell'] = array($row->clave_producto,$row->nombre_producto,$row->proveedor,
		$row->nombre_medida,$row->cantidad_actual,$row->precio,$row->status);
		$i++;
	}     
